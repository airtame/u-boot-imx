...
void __led_toggle (led_id_t mask)
{
    ...
    if (!toggle_gpio && STATUS_LED_BIT & mask)
        toggle_gpio = AIRTAME_LED_GPIO;
#endif
    if (toggle_gpio) {
        if (!gpio_request(toggle_gpio, "")) {
            gpio_direction_input(toggle_gpio);
...
void __led_set (led_id_t mask, int state)
{
    ...
    if (STATUS_LED_BIT & mask) {
        if (!gpio_request(AIRTAME_LED_GPIO, "")) {
            gpio_direction_output(AIRTAME_LED_GPIO, 0);
            gpio_set_value(AIRTAME_LED_GPIO, state);
        }
    }
#endif
}
